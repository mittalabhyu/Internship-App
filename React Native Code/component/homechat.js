...
export function homechat({ navigation }) {
    const submitData = async(name) => {
        if (name < global.draweruname) {
            global.chatroom = name.concat(global.draweruname);
        } else {
            global.chatroom = global.draweruname.concat(name);
        }
       const ab=await fetch(global.apii+"chatroom/" + global.chatroom, {
            method: "get",
            headers: {
            ...
            .then(res => res.json()).then(data => {
                ...
                if(data.length!=0)
                {
                    ...
                    {
         global.msg=data[0].message.reverse();
                } 
            }    
            })
        navigation.navigate("Message", { name: global.chatroom });
    }
    ...
    return (
        <View style={styles.container}>
            ...
            <FlatList
                ...
                data={global.namee}
                renderItem={itemss => (
                    <Card style={{ backgroundColor: 'orange', borderRadius: 20 }} onPress={() => submitData(itemss.item["username"])}>
