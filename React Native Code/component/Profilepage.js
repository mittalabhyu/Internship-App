import 'react-native-gesture-handler';
import React from 'react';
import { StyleSheet, Text, View, TouchableOpacity, ScrollView } from 'react-native';
import { Card, CardTitle, CardContent, CardAction, CardButton, CardImage } from 'react-native-material-cards'
import {
    Avatar
} from 'react-native-paper';
import { FlatList } from 'react-native-gesture-handler';
import './global';
export function communityscreenadd() {
    const acceptrequest=(id)=>{
        fetch(global.apii+"accept-req", {
            method: "post",
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                uid: global.valuee,
                fid: id
            })
        })
            .then(res => res.json()).then(data => {
            //   console.log("accept req data ",data);
            })
            Alert.alert("Request Accepted!!");
    }
    fetch(global.apii +"matchmaking", {
        method: "post",
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({
            uid: global.valuee
        })
    })
        .then(res => res.json()).then(data => {
            global.namee = data;
            console.log("Matchmaking data ",data);
            
        })
        fetch(global.apii + "event-display", {
            method: "get",
            headers: {
                'Content-Type': 'application/json'
            },
        })
            .then(res => res.json()).then(data => {
                global.event = data;
               // console.log("Event data ",data);
            })
            fetch(global.apii + "pending-req/"+global.valuee, {
                method: "get",
                headers: {
                    'Content-Type': 'application/json'
                },
            })
                .then(res => res.json()).then(data => {
                  //  console.log("Pending req ",data);
                    global.pending=data;
                })
                fetch(global.apii + "friend-list/"+global.valuee, {
                    method: "get",
                    headers: {
                        'Content-Type': 'application/json'
                    },
                })
                    .then(res => res.json()).then(data => {
                    //    console.log("FriendList ",data);
                        global.friendlist=data;
                    })
                    /*
                     fetch(global.apii +"list-data", {
        method: "post",
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({
            list: list of user id
        })
    })
        .then(res => res.json()).then(data => {
            
            console.log("Listdata ",data);
            
        })
                    */
    return (
        <View style={styles.container}>
            <Avatar.Image source={{
                uri: 'data:image/jpeg;base64,'
            }}
                margin={10}
                size={100}
            />
            <View style={{ flex: 5, width: '90%' }}>
                <Card style={{ backgroundColor: 'orange', borderRadius: 10 }}>
                    <CardTitle
                        style={{ justifyContent: 'center' }}
                        title={global.drawername + " " + global.dataa["response"]["lastname"]}
                        titleStyle={{ alignSelf: 'center', fontWeight: 'bold' }}
                    />
                    < Text style={{ color: 'white', alignSelf: 'center', fontSize: 20, fontWeight: 'bold' }}></Text>
                    <Text style={{ color: 'white', alignSelf: 'center', marginBottom: 6, fontSize: 20, fontWeight: 'bold' }}>{"@" + global.draweruname} </Text>
                    <Text style={{ color: 'black', alignSelf: 'center', marginVertical: 6, fontSize: 20, fontWeight: 'bold' }}>Email :</Text>
                    <Text style={{ color: 'white', alignSelf: 'center', marginVertical: 6, fontSize: 20, fontWeight: 'bold' }}>{global.dataa["response"]["email"]} </Text>
                    <Text style={{ color: 'white', alignSelf: 'center', marginVertical: 6, fontSize: 20, fontWeight: 'bold' }}>{"City :  " + global.dataa["response"]["city"]} </Text>
                    <Text style={{ color: 'white', alignSelf: 'center', marginVertical: 6, fontSize: 20, fontWeight: 'bold' }}>{"Country : " + global.dataa["response"]["country"]} </Text>
                    <Text style={{ color: 'white', alignSelf: 'center', marginVertical: 6, fontSize: 20, fontWeight: 'bold' }}>{"Contact : " + global.dataa["response"]["contact"]} </Text>
                    <CardAction
                        separator={true}
                        inColumn={false}
                        style={{ justifyContent: 'center' }}>
                        <Text style={{ color: 'black', alignSelf: 'center', marginVertical: 10, fontSize: 22, fontWeight: 'bold' }}>Intrests...</Text>
                    </CardAction>
                </Card>
            </View>
            <FlatList
                horizontal
                flex={1}
                width="90%"
                data={global.dataa["response"]["intrest"]}
                renderItem={items => (
                    <TouchableOpacity
                        onPress={() => { }}
                        style={{ backgroundColor: "red", borderRadius: 25, width: 120, height: 40, marginHorizontal: 20, justifyContent: 'center', fontWeight: 'bold' }}>
                        <Text style={{ fontSize: 20, color: 'white', textAlign: 'center' }}>{items.item}</Text>
                    </TouchableOpacity>
                )} />
                <Text style={{textAlign:'center',fontFamily:'JosefinSans-SemiBold',fontSize:20,marginHorizontal:10,color:'#086688'}}>Connection Requests</Text>
                {global.pending==""?
                <Text style={{textAlign:'center',fontFamily:'JosefinSans-Regular',fontSize:15,marginHorizontal:10,color:'#FFAD2F'}}>No New Request</Text>
                :
                <FlatList
                flex={1}
                width="90%"
                data={global.pending}
                renderItem={items => (
                    <Card style={{ backgroundColor: 'orange', borderRadius: 10 }}>
                    <CardTitle
                        style={{ justifyContent: 'center' }}
                        title={items.item}
                        titleStyle={{ alignSelf: 'center', fontWeight: 'bold' }}
                    />
                    
                    <TouchableOpacity
                        onPress={() => {acceptrequest(items.item) }}
                        style={{ alignSelf: 'center', backgroundColor: "#FDFFFC", height: 40,paddingHorizontal:5, marginHorizontal: 20, marginVertical: 5, justifyContent: 'center', fontWeight: 'bold' }}>
                                        <Text style={{ fontSize: 20, color: '#086688', textAlign: 'center', alignSelf: 'center',fontFamily:'JosefinSans-SemiBold' }}>Accept Request</Text>
                    </TouchableOpacity>
                    </Card>
                )} />}
        </View>
    )
}
const styles = StyleSheet.create({
    container: {
        flex: 1,
        backgroundColor: '#fff',
        alignItems: 'center',
        justifyContent: 'center',
    },
});
