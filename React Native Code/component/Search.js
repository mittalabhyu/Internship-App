import 'react-native-gesture-handler';
import CustomIcon from './CustomIcon';
import { Card, CardTitle, CardContent, CardAction, CardButton, CardImage } from 'react-native-material-cards'
import React, { useEffect, useState } from 'react';
import { StyleSheet, Text, View, Image, TextInput, Icon,Button, ImageBackground, Alert, TouchableOpacity, ScrollView, FlatList } from 'react-native';
import './global';
import {
    Avatar
} from 'react-native-paper';
export function Search() {
    //var textInputComponents = global.namee.map(type => <Text style={{fontSize:20,color:'blue'}}>{type}</Text> )
    const sendreq=(id)=>{
        fetch(global.apii+"send-req", {
            method: "post",
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                uid: global.valuee,
                fid: id
            })
        })
            .then(res => res.json()).then(data => {
               console.log("send req data ",data);
            })
            Alert.alert("Request Sent Successfully!!");
    }
    const places=[1,2,3,4,5,6]
    return (
        <View style={{flex:1}}>
        <ScrollView style={styles.container}>
            <Text style={{ fontSize: 17, color: 'black',alignSelf:'center',fontFamily:'JosefinSans-Bold',marginVertical:1 }}>Build Connections</Text>
            <Text style={{ fontSize: 14, color: 'black',alignSelf:'center',fontFamily:'JosefinSans-Regular'}}>Click on the profile to connect</Text>
             {places.map((place) => {
            return (
                <Card style={{ backgroundColor: 'white', borderRadius: 8,margin:6,elevation:6,paddingHorizontal:'2%',alignSelf:'center',
                shadowOffset: {
                    width: 1,
                    height: 1,
                },
                shadowOpacity: 0.5,
                shadowRadius: 4,
                shadowColor: "#000" }} onPress={() => Alert.alert("Request Sent")}>
                <View style={{flexDirection:'row',paddingTop:12}}>
                <Avatar.Image source={{
                uri: 'data:image/jpeg;base64,'
            }}
                margin={2}
                size={45}
            />
                <Text style={{ marginTop:10,marginLeft:5, fontFamily:'JosefinSans-SemiBold',fontSize:15 ,paddingRight:50}}> Tony Sheldon, Delhi</Text>
                <TouchableOpacity
        // onPress={() => {loginHandle( data.username, data.password )}}
      
        //onPress={() => setVisibility(true)}
        style={{ marginTop:5, backgroundColor: '#FFAD2F',borderRadius: 17,width:34,height:34,justifyContent: 'center'}}>
        <Text style={{ fontFamily:'JosefinSans-Regular',fontSize: 11, color: 'white' ,justifyContent: 'center', alignSelf:'center',paddingRight:3}}> 39%</Text>
    </TouchableOpacity>
                </View>
                <View style={{flexDirection:'row'}}>
                <TouchableOpacity
        // onPress={() => {loginHandle( data.username, data.password )}}
      
        //onPress={() => setVisibility(true)}
        style={{ shadowColor: "#000",
        shadowOffset: {
            width: 0,
            height: 2,
        },
        shadowOpacity: 0.5,
        shadowRadius: 4,
        
        elevation: 7, backgroundColor: 'black',borderRadius: 25,paddingHorizontal:10, margin:10,justifyContent: 'center' , fontWeight: 'bold' }}>
        <Text style={{ fontFamily:'JosefinSans-SemiBold',fontSize: 14,paddingRight:5,paddingBottom:3, color: 'white' ,alignSelf:'center'}}> Singing</Text>
    </TouchableOpacity>
    <TouchableOpacity
        // onPress={() => {loginHandle( data.username, data.password )}}
      
        //onPress={() => setVisibility(true)}
        style={{ shadowColor: "#000",
        shadowOffset: {
            width: 0,
            height: 2,
        },
        shadowOpacity: 0.5,
        shadowRadius: 4,
        
        elevation: 7, backgroundColor: 'black',borderRadius: 25, paddingHorizontal:10,margin:10,justifyContent: 'center' , justifyContent: 'center', fontWeight: 'bold' }}>
        <Text style={{ fontFamily:'JosefinSans-SemiBold',fontSize: 14,paddingRight:5,paddingBottom:3, color: 'white' ,alignSelf:'center'}}> Blockchain</Text>
    </TouchableOpacity>
    <TouchableOpacity
        // onPress={() => {loginHandle( data.username, data.password )}}
      
        //onPress={() => setVisibility(true)}
        style={{ shadowColor: "#000",
        shadowOffset: {
            width: 0,
            height: 2,
        },
        shadowOpacity: 0.5,
        shadowRadius: 4,
        
        elevation: 7, backgroundColor: 'black',borderRadius: 25, paddingHorizontal:10,margin:10,justifyContent: 'center' , fontWeight: 'bold' }}>
        <Text style={{ fontFamily:'JosefinSans-SemiBold',fontSize: 14,paddingRight:5,paddingBottom:3, color: 'white' ,alignSelf:'center'}}> AI</Text>
    </TouchableOpacity>
    
    
                </View>
            </Card>
            )
         })}
        
            <FlatList
                flex={1}
                width="98%"
                data={global.namee}
                renderItem={itemss => (
                    <Card style={{backgroundColor: 'white', borderRadius: 8,margin:10,elevation:6,paddingHorizontal:'2%',alignSelf:'center',
                shadowOffset: {
                    width: 1,
                    height: 1,
                },
                shadowOpacity: 0.5,
                shadowRadius: 4,
                shadowColor: "#000" }} onPress={() => {sendreq(itemss.item["_id"]) }}>
                 <View style={{flexDirection:'row',paddingTop:12}}>
                <Avatar.Image source={{
                uri: 'data:image/jpeg;base64,'
            }}
                margin={2}
                size={45}
            />
                <Text style={{ marginTop:10,marginLeft:5, fontFamily:'JosefinSans-SemiBold',fontSize:15 ,paddingRight:50}}> {itemss.item["firstname"] + " " + itemss.item["lastname"]+", "+itemss.item["city"]}, Delhi</Text>
                <TouchableOpacity
        style={{ marginTop:5, backgroundColor: '#FFAD2F',borderRadius: 17,width:34,height:34,justifyContent: 'center'}}>
        <Text style={{ fontFamily:'JosefinSans-Regular',fontSize: 11, color: 'white' ,justifyContent: 'center', alignSelf:'center',paddingRight:3}}> 39%</Text>
    </TouchableOpacity>
                </View>
                        <CardAction
                            separator={true}
                            inColumn={false}>
                            <FlatList
                                flex={1}
                                width="90%"
                                data={itemss.item["intrest"]}
                                renderItem={items => (
                                    <TouchableOpacity
       
        style={{ shadowColor: "#000",
        shadowOffset: {
            width: 0,
            height: 2,
        },
        shadowOpacity: 0.5,
        shadowRadius: 4,
        
        elevation: 7, backgroundColor: 'black',borderRadius: 25, paddingHorizontal:10,margin:10,justifyContent: 'center' , justifyContent: 'center', fontWeight: 'bold' }}>
        <Text style={{ fontFamily:'JosefinSans-SemiBold',fontSize: 14,paddingRight:5,paddingBottom:3, color: 'white' ,alignSelf:'center'}}> {items.item}</Text>
    </TouchableOpacity>
                                
                                )} />
                        </CardAction>
                       
                    </Card>
                                )} />
      </ScrollView>
        <View style={styles.foot}>
        <CustomIcon name='share' color="black" size={35} style={{paddingLeft:'19%',paddingTop:'1%',paddingRight:'3%'}}/>
            <Text style={{ fontSize: 17, color: 'black',alignSelf:'center',fontFamily:'JosefinSans-SemiBold',paddingVertical:'3%'}}>Get more connections</Text>
           </View>
           
           </View>
    )
}
const styles = StyleSheet.create({
    container: {
        flex: 1,
        backgroundColor:'white',
        
       
    },
    
    foot:{
        height:'8%',width:'120%',
        flexDirection:'row',
         top:3,
         shadowColor:'#000',
        shadowOffset:{width:0,height:5},
         shadowOpacity:0.9,
         shadowRadius:5,
         elevation:3,
         bottom: 0,
         left: -32,
         right: 0,
         zIndex: 1,
        backgroundColor: 'white',
    }
});